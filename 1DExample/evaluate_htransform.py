...
from neural_operator import CondFNO, FNO
...
load_path_htrans = f"h_transform/model_type={model_type}/alpha={power}/"

save_path = Path(f"results/h_transform/model_type={model_type}/alpha={power}")
...
model.load_state_dict(torch.load(os.path.join(load_path, "ema_model.pt"), map_location=device, weights_only=False)["shadow"])
model.to(device)
model.eval()
...
h_trans.load_state_dict(torch.load(os.path.join(load_path_htrans, "model.pt"), weights_only=True))
h_trans.to(device)
h_trans.eval()

sde = OU(beta_min=base_config["sde"]["beta_min"], beta_max=base_config["sde"]["beta_max"])
...
def sample_conditional(y, num_samples_K,pos):
    ...
    for ti in tqdm(reversed(ts), total=len(ts)):
        ...
        with torch.no_grad():
            score, _ = cond_model(xt, y, t, pos_inp)
