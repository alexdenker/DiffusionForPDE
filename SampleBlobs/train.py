...
from noise_sampler import RBFKernel
from sde import OU
...
from configs import WANDB_PROJECT, WANBD_ENTITY
...
with wandb.init(**wandb_kwargs) as run:
    ...
    for epoch in range(configs["num_epochs"]):
        ...
        for idx, x in tqdm(enumerate(data_loader), total=len(data_loader)):
            ...
            pred = model_fn(xt, random_t, pos_inp) # last layer / (1 - exp(-t)).sqrt()

            # TODO: Choose loss norm 
            loss = torch.sum((cov_t*pred - z)**2)/pred.shape[0] # loss function (mulitplied by (1-exp(-t)).sqrt()
            loss.backward() 
        ...
        with torch.no_grad():
            ...
            for idx in range(plot_batch.shape[0]):
                ...
                axes[1,idx].set_title(f"Noisy phantom at t={times[idx].item()}")
                axes[1,idx].axis("off")
